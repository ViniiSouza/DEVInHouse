...
function App() {
  ...
  const [nomeUsuario, setNomeUsuario] = useState('viniisouza');
  ...
  return (
    <div className="App">
      ...
      <Summary imgSrc={`https://github.com/${nomeUsuario}.png`} altText={`Imagem perfil ${nomeUsuario}`} />
          ...
          (
            <div id="repo-list">
              {repositorios.map((repo) =>
                <Repository key={repo.id} titulo={repo.titulo} descricao={repo.descricao} destacar={repo.id === idSelecionado} turnDestaque={() => setIdSelecionado(repo.id)} />
              )}
            </div>
          )
        }
